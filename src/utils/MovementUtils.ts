...
export class MovementUtils {
    ...
    public static generalGatherMovement(creep: Creep, controllerLink: StructureLink | undefined = undefined, targetSource: any = undefined) {
        ...
        const nearestStoreStructure = creep.pos.findClosestByPath(FIND_STRUCTURES, {
            filter: (struc) => {
                return (struc.structureType === STRUCTURE_CONTAINER || struc.structureType === STRUCTURE_TERMINAL || struc.structureType === STRUCTURE_STORAGE) && struc.store.energy > 0
            }
        ...
        if(creep.memory.role === 'upgrader' || creep.memory.role === 'builder') {
            ...
            if(commandLevel >= 4 && creep.memory.role == 'upgrader' && creep.room.controller && creep.pos.inRangeTo(creep.room.controller.pos.x,creep.room.controller?.pos.y,2)){
        ...
        if(droppedSources && droppedSources.amount >= creep.store.getCapacity() && creep.pickup(droppedSources) == ERR_NOT_IN_RANGE){
            creep.moveTo(droppedSources, {visualizePathStyle: {stroke: '#ffaa00'}});
        }else if(container && container.store.energy >= creep.store.getCapacity() && creep.withdraw(container, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
            creep.moveTo(container, {visualizePathStyle: {stroke: "#ffffff"}});
        ...
        else if(terminal && terminal.store[RESOURCE_ENERGY] > 3000 && creep.withdraw(terminal , RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
            creep.moveTo(terminal);
        }
        else if (target_storage && target_storage.store[RESOURCE_ENERGY] > 3000 && target_storage.room.energyAvailable === target_storage.room.energyCapacityAvailable && creep.withdraw(target_storage, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
            creep.moveTo(target_storage, {visualizePathStyle: {stroke: "#ffffff"}});
        }
    ...
    private static generalScientistGather(creep:Creep, terminal:StructureTerminal, commandLevel: number, labs:StructureLab[], target_storage: StructureStorage, nearestStorageOrTerminal:StructureTerminal | StructureStorage | null): boolean {
