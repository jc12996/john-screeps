  ...
  public static run(creep: Creep): void {
    ...
    const nearestStorageOrTerminal: StructureTerminal | StructureStorage | null = creep.pos.findClosestByPath(
      FIND_STRUCTURES,
      {
        filter: structure => {
          return (
            ((structure.structureType === STRUCTURE_TERMINAL && structure.store[RESOURCE_ENERGY] < 100000) ||
              (structure.structureType === STRUCTURE_STORAGE && structure.store[RESOURCE_ENERGY] < 900000)) &&
            structure.store.getFreeCapacity(RESOURCE_ENERGY) > 0 &&
            structure.room?.controller?.my
          );
        }
